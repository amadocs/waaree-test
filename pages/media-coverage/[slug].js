...
const NewsDetailSection = dynamic(() =>  import( '../../Components/InnerPagesSection/NewsDetailSection'))
...
export async function getServerSideProps({res,query}) {
  ...
  let {slug} = query
  const res_1 =  await fetch(process.env.NEXT_PUBLIC_API_URL + `newses?populate=*&filters[slug]=${slug}`
  ...
  const response =  await fetch(process.env.NEXT_PUBLIC_API_URL + `pages?populate=*&filters[slug]=media-coverage`
