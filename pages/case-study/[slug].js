...
const CaseStudyDetail = dynamic(() =>  import( '../../Components/InnerPagesSection/CaseStudyDetail'))
...
export async function getServerSideProps({res,query}) {
  ...
  let {slug} = query
  const res_1 =  await fetch(process.env.NEXT_PUBLIC_API_URL + `case-studies?populate=*&filters[slug]=${slug}`
  ...
  const response =  await fetch(process.env.NEXT_PUBLIC_API_URL + `pages?populate=*&filters[slug]=case-study`
