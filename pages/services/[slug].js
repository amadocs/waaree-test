...
const ImageGallarySection = dynamic(() =>  import( '../../Components/InnerPagesSection/ImageGallarySection'))
const ServiceInternalDetailSection = dynamic(() =>  import( '../../Components/InnerPagesSection/ServiceInternalDetailSection'))
const ImageGallaryFourCol = dynamic(() =>  import( '../../Components/InnerPagesSection/ImageGallaryFourCol'))
const ServiceTwoColSection = dynamic(() =>  import( '../../Components/InnerPagesSection/ServiceTwoColSection'))
...
export async function getServerSideProps({res,query}) {
    ...
    let {slug} = query
    const res_1=  await fetch(process.env.NEXT_PUBLIC_API_URL + `services?populate=*&filters[slug]=${slug}`
 ...
 const social_data = await social.json()
    ...
    return { props: { data : temp.data, contact_icons:contact_data.data, cookies : cookie_data.data, More_Menu:menu_data.data,footer_contact:footer_data.data,social_icons:social_data.data , contact_icons:contact_data.data} }
  }
  
  
  
  export default function Slug({data,contact_icons,cookies,More_Menu,footer_contact ,social_icons}){
  ...
  <Meta
        ...
        <InnerBannerSection
          bgSrc={data[0]?.attributes?.Banner_Image
            ? data[0]?.attributes?.Banner_Image?.data?.attributes?.url
            : process.env.NEXT_PUBLIC_NO_IMAGE}
          pageTitle={"Services - " + `${data[0]?.attributes?.Title}`}
          bannerText={data[0]?.attributes?.Short_Description} />
        {
            !data[0]?.attributes?.Service_Description.length ? "" :
            <ServiceTwoColSection data={data[0]} />
        }
        {
            !data[0]?.attributes?.Service_List.length ? "" :
            <ServiceInternalDetailSection data={data[0]} />
        }

        {
            !data[0]?.attributes?.Project_Gallery.length ? "" :    
            <ImageGallarySection 
            gallarySectionTitle={data[0]?.attributes?.Key_Projects_Title} 
            gallaryType={<ImageGallaryFourCol data={data[0]} />}/>  
